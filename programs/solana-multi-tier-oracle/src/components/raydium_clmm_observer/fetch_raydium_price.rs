...
use crate::components::raydium_clmm_observer::twap::{find_observation_for_window, twap_tick_from_cumulatives, t2ema_tick, confidence_from_variance, assess_manipulation_risk};
use crate::components::raydium_clmm_observer::raydium_accounts::{verify_observation_pda_and_read_pool, read_observation};
use crate::components::raydium_clmm_observer::sqrt_price_to_tick::{ui_price_from_sqrt_q64, get_sqrt_ratio_at_tick};
...
pub fn fetch_raydium_price_from_observations(
    ...
    let pool = verify_observation_pda_and_read_pool(pool_account_info, observation_account_info, program_id)?;
    ...
    let (index_then, index_now, seconds_elapsed) = find_observation_for_window(&observation, params.timestamp, params.window_seconds)?;
    ...
    require!(dev_vs_current <= params.max_tick_deviation, RaydiumObserverError::ExcessiveDeviation);
    ...
    require!(dev_twap_vs_t2ema <= params.max_tick_deviation, RaydiumObserverError::ExcessiveDeviation);
    ...
    let risk_score = assess_manipulation_risk(base_confidence, dev_vs_current, seconds_elapsed, pool.liquidity(), params.min_liquidity);
