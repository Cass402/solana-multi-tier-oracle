...
use crate::{components::{twap, ui_price_from_sqrt_q64}, state::{
    governance_state::{GovernanceState, Permissions}, historical_chunk::{HistoricalChunk, PricePoint}, oracle_state::{OracleState, PriceData}, price_feed::{FeedFlags, SourceType}
...
use crate::components::raydium_clmm_observer::{
    ...
    raydium_constants::{RAYDIUM_CLMM_PROGRAM_ID_DEVNET, RAYDIUM_CLMM_PROGRAM_ID_MAINNET, OBSERVATION_SEED, OBSERVATION_UPDATE_DURATION},
};
use crate::error::{StateError, RaydiumObserverError};
use crate::utils::constants::{ORACLE_STATE_SEED, GOVERNANCE_SEED, HISTORICAL_CHUNK_SEED, BUFFER_SIZE, MAX_TWAP_WINDOW, MIN_HISTORICAL_INTERVAL};
...
fn stream_twap_from_chunks(
    ...
    let mut visit_chunk = |chunk: &HistoricalChunk, chunk_name: &str, events_counter: &mut u32| -> Result<()> {
        if chunk.count == 0 {
            return Ok(());
        }
        ...
        for _ in 0..chunk.count {
            ...
            if let Some(prev_point) = previous_point {
                ...
                let price_weighted = (prev_point.price as i128).checked_mul(combined_weight as i128);
                let new_price_sum = price_weighted.and_then(|pw| weighted_price_sum.checked_add(pw));

                // Use time-only weighting for confidence calculation
                let conf_time_weighted = (conf_sample as u128).checked_mul(time_delta);
                let new_conf_sum = conf_time_weighted.and_then(|ctw| conf_time_sum.checked_add(ctw));
                let new_time_weight = time_only_weight.checked_add(time_delta);

                let new_total_weight = total_weight.checked_add(combined_weight);

                match (new_price_sum, new_conf_sum, new_total_weight, new_time_weight) {
                    ...
                    _ => {
                        ...
                        conf_time_sum = conf_time_sum.saturating_add(
                            (conf_sample as u128).saturating_mul(time_delta)
                        );
    ...
    if let Some(last_point) = previous_point {
        ...
        if dt > 0 {
            ...
            let last_price_weighted = (last_point.price as i128)
                .checked_mul(last_combined_weight as i128);
            let final_price_sum = last_price_weighted
                .and_then(|lpw| weighted_price_sum.checked_add(lpw));

            let last_conf_time_weighted = (last_conf_sample as u128)
                .checked_mul(last_time_weight);
            let final_conf_sum = last_conf_time_weighted
                .and_then(|lctw| conf_time_sum.checked_add(lctw));
            ...
            match (final_price_sum, final_conf_sum, final_total_weight, final_time_weight) {
                (Some(ps), Some(cs), Some(tw), Some(tw_time)) => {
                    weighted_price_sum = ps;
                    conf_time_sum = cs;
                    total_weight = tw;
                ...
                _ => {
                    ...
                    weighted_price_sum = weighted_price_sum.saturating_add(
                        last_point.price.saturating_mul(last_combined_weight as i128)
                    );
                    conf_time_sum = conf_time_sum.saturating_add(
...
pub fn update_price(
    ...
    let decimal_price = fetch_raydium_price_from_observations(&ctx.accounts.raydium_pool, &ctx.accounts.raydium_observation, raydium_program_id, params)?;
    ...
    let twap_result = if is_first_run {
        ...
        stream_twap_from_chunks(&[oldest, middle, newest], oracle_twap_window, current_time, &ctx.accounts.oracle_state.key())?
    };

    if let Some(feed_index) = oracle_state.price_feeds
        .iter()
        .position(|feed| feed.source_address == *ctx.accounts.raydium_pool.key)
    {
        ...
        feed.volume_24h = 0;
        feed.liquidity_depth = core::cmp::min(decimal_price.liquidity_depth, i128::MAX as u128) as i128;
    ...
    let chunks = (&*current_historical_chunk, &*historical_chunk_1, &*historical_chunk_2);
    let (active_chunk_index, needs_rotation) = determine_active_chunk(chunks, oracle_state.current_chunk_index)?;
